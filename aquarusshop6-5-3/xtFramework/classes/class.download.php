...
class download {
	...
	protected function logDownload($mediaId) {
		...
		$query = "SELECT * FROM ".TABLE_MEDIA." WHERE id=?";
		$rs = $db->Execute($query, [(int)$mediaId]);
		
		if ($rs->RecordCount()>0) {
			$downloadCount = 0;
			
			// Check if there is any previous log about this file, to get actual download count because real one can be reseted
            $query = "SELECT * FROM ".TABLE_DOWNLOAD_LOG." WHERE orders_id=? AND media_id=? AND download_action=1 ORDER BY download_log_id DESC LIMIT 1";
			$log = $db->Execute($query, [(int)$orderId, (int)$mediaId]);
            ...
            $query = "SELECT * FROM ".TABLE_ORDERS_PRODUCTS_MEDIA." WHERE orders_id=? AND media_id=?";
			$order_media = $db->Execute($query, [(int)$orderId, (int)$mediaId]);
	...
	function _createSymlink($file,$tmp_dir) {
		...
		if(is_link($dir.$this->public_dir . $tmp_dir . "/" . $file) || is_file($dir.$this->public_dir . $tmp_dir . "/" . $file))
		{
			//error_log('symlink : file exists');
			return;
		}
		if ( PHP_OS == "WINNT" ) {
            if(!function_exists('link')){
                $log_data = array();
                $log_data['error'] = 'function link not available';
                $logHandler->_addLog('error','download','0',$log_data);
            } else {
             ...
             if(!function_exists('link')) {
                 $log_data = array();
                 $log_data['error'] = 'function symlink not available';
                 $logHandler->_addLog('error', 'download', '0', $log_data);
             } else {
	...
	function _addSymlinkLog($dir) {
        ...
        $sql = "SELECT * FROM ".TABLE_MEDIA_SYMLINK." WHERE symlink_dir = '".$dir."'";
        $rs = $db->Execute($sql);
        if ($rs->RecordCount()==0) {
            $path_array = array();
    ...
    */
	...
	{
		$dir2create = _SRV_WEBROOT.$this->public_dir.$dir;
		if (file_exists($dir2create) && !is_dir($dir2create))
		{
			//error_log($dir2create .' is a file. cannot create directory');
			return;
		}
		if (is_dir($dir2create))
		{
			umask(0000);
        	chmod($dir2create,0777);
			return;
		}
		umask(0000);
        mkdir($dir2create,0777);
	}
	...
	function _checkDowloadAllowed($date,$days=0,$count_max=0,$count_loaded=0) {
        ...
        public function checkDownloadPermission($mediaId, $downloadStatus)
        {
            ...
            $query = "SELECT distinct m.* FROM " .TABLE_MEDIA. " m ". $permission->_table." where m.id=? ".$permission->_where." and m.download_status=? and m.status='true'";
            $record = $db->Execute($query, [(int)$mediaId, $downloadStatus]);
        ...
        public function gotoErrorDownload($linkList)
        {
            ...
            $info->_addInfoSession(__text('TEXT_NO_DOWNLOAD_IN_LANG'),'error');
