...
class MediaImageList extends MediaImages
{
    ...
    function _getParams ()
    {
        ...
        if ($this->url_data['pg'] == 'overview' && !$this->url_data['link_id'] && !($this->url_data['new'] || $this->url_data['edit'])) {
            ...
            if (!($this->url_data['new'] && $this->url_data['edit'])) {
                ...
                $unlink_js = $extF->_MultiButton_stm('TEXT_UNLINK', 'image_unlink');
                $rowActionsFunctions['unlink'] = $unlink_js;
                $rowActions[] = array('iconCls' => 'sort_up', 'qtipIndex' => 'qtip1', 'tooltip' => __text('TEXT_SORT_UP'));
                $rowActions[] = array('iconCls' => 'sort_down', 'qtipIndex' => 'qtip1', 'tooltip' => __text('TEXT_SORT_DOWN'));
                $rowActions[] = array('iconCls' => 'image_processing', 'qtipIndex' => 'qtip1', 'tooltip' => __text('TEXT_IMAGE_PROCESSING'));
                $rowActions[] = array('iconCls' => 'unlink', 'qtipIndex' => 'qtip1', 'tooltip' => __text('TEXT_UNLINK'));
                $params['rowActions'] = $rowActions;
                $params['rowActionsFunctions'] = $rowActionsFunctions;

                $UserButtons['unlink'] = array('text' => 'TEXT_UNLINK', 'style' => 'unlink', 'icon' => 'unlink.png', 'font-icon'=>'fa fa-unlink', 'acl' => 'edit', 'stm' => $unlink_js);
            }
        ...
        ($plugin_code = $xtPlugin->PluginCode('class.MediaImageList.php:_getParams')) ? eval($plugin_code) : false;
    ...
    function _get ($ID = 0)
    {
        ...
        ($plugin_code = $xtPlugin->PluginCode('class.MediaImageList.php:_get_bottom')) ? eval($plugin_code) : false;
    ...
    {
        global $db, $language, $filter, $seo, $db, $xtPlugin;
        ...
        ($plugin_code = $xtPlugin->PluginCode('class.MediaImageList.php:_set_bottom')) ? eval($plugin_code) : false;
    ...
    function _unset ($id = 0)
    {
        ...
        if ($checkMainFile)
        {
            $this->unset_internal_mainFile( $this->url_data['currentType'], $fileName);
    ...
    private function unset_internal_mainFile($className, $fileName)
    {
        global $db;

        $clz = new $className();
        $sql = "SELECT DISTINCT ".$clz->_master_key." FROM ".$clz->_table." WHERE ".$clz->_image_key."=?";
        $rows = $db->GetArray($sql, array($fileName));
        foreach($rows as $row)
        {
            $this->unsetMainFile(false, $row[$clz->_master_key], $this->url_data['currentType']);
        }
    }


    function _imageUnlink ($id = 0)
    {
        global $db;
        if ($id == 0) return false;
        if ($this->position != 'admin') return false;
        if(is_array($id))
        {
            $ids = explode(',', $this->url_data['m_ids']);
