...
class MediaList extends xt_backend_cls
{
    ...
    function _getParams ()
    {
        ...
        if ($this->url_data['mgID']) {
            ...
            $i_js .= "addTab('row_actions.php?type=image_importing&seckey="._SYSTEM_SECURITY_KEY."&currentType='+currentType+'&mgID='+mgID,'" . __text('TEXT_IMPORT') . "')";
            ...
            if($this->url_data['galType']!='files_free' && $this->url_data['galType']!='files_order')
            {
            $sjs = "var edit_id = " . $this->url_data['mgID'] . ";";
            $sjs .= "addTab('row_actions.php?type=image_processing&seckey="._SYSTEM_SECURITY_KEY."&mgID='+edit_id,'" . __text('TEXT_IMAGE_PROCESSING') . "')";
        ...
        $menuGroups[] = array('group' => 'edit_data', 'group_name' => __text('TEXT_EDIT'), 'ToolbarPos' => 'Toolbar', 'Pos' => 'grid','iconCls'=>'fas fa-wrench'); // Toolbarpos = TopToolbar or Toolbar, Pos = grid / edit / both
        ...
        ($plugin_code = $xtPlugin->PluginCode('class.MediaList.php:_getParams_bottom')) ? eval($plugin_code) : false;
    ...
    function _getMGData ($id)
    {
        ...
        $record = $db->Execute("SELECT DISTINCT m_id FROM " . TABLE_MEDIA_TO_MEDIA_GALLERY . " WHERE mg_id=?", array($id));
    ...
    function _getExistingMGData ($link_id, $class, $type)
    {
        global $db;

        $ids = array();

        $record = $db->Execute("SELECT DISTINCT m_id FROM " . TABLE_MEDIA_LINK . " WHERE link_id=? AND class=? AND type=?", array($link_id, $class, $type));
        if ($record->RecordCount() > 0) {
            while (!$record->EOF) {
                $ids[] = $record->fields['m_id'];
                $record->MoveNext();
    ...
    function _get ($ID = 0)
    {
        ...
        if (count($ids) > 0) {
            ...
            $qry .= " and ".$this->_table.".id IN (".implode(',', $ids).") ";
        }

        if ($this->url_data['mgID']) {
            $mg_ids = $this->_getMGData($this->url_data['mgID']);
            if($this->url_data['products_id'] && in_array($this->url_data['mgID'], array(6,7)) && is_array($mg_ids) && count($mg_ids))
            {
                $existing_mg_ids = $this->_getExistingMGData($this->url_data['products_id'], 'product', 'media');
                $mg_ids = array_diff($mg_ids, $existing_mg_ids);
            }
        ...
        if ($is_data) {
            ...
            if ($this->url_data['get_data']) {
				...
				if (($this->url_data['mgID']==6) or ($this->url_data['mgID']==7))
				{
                    $data[0]['group_permission_info2']=_getPermissionInfo();
                    define('TEXT_GROUP_PERMISSION_INFO2', __text('TEXT_GROUP_PERMISSION_INFO'));
