...
class category extends xt_backend_cls {
    ...
    function getCategoryData ($catID = 0) {
        global $xtPlugin, $db, $language, $current_manufacturer_id,$mediaImages;
        ...
        if ($catID == 0 && empty($current_manufacturer_id)) {
            $data['listing_template'] = defined('_STORE_TEMPLATE_PRODUCT_LISTING_STARTPAGE') ? constant('_STORE_TEMPLATE_PRODUCT_LISTING_STARTPAGE'):'NODEF__STORE_TEMPLATE_PRODUCT_LISTING_STARTPAGE';
            return $data;
        } elseif ($catID == 0 && !empty($current_manufacturer_id)) {
            $data['listing_template'] = defined('_STORE_TEMPLATE_PRODUCT_LISTING_MANUFACTURERS') ? constant('_STORE_TEMPLATE_PRODUCT_LISTING_MANUFACTURERS'):'NODEF__STORE_TEMPLATE_PRODUCT_LISTING_MANUFACTURERS';
            return $data;
        }
        ...
        if($record->RecordCount() > 0){
            ...
            {
                $md = new MediaData();
                $media_id = $md->_getMediaID($data['categories_image'], 'category');
                $data['categories_image_data'] = $md->getMediaData($media_id);
            ...
            $plugin_code = $xtPlugin->PluginCode('category:getCategoryData_bottom');
    ...
    function getCategoryListingAdmin ($catID = '', $store='', $unassigned=false) {
        ...
        if ($unassigned)
        {
            if ($store_handler->store_count==1) return false;
        ...
        if ($store!='')
        {
            ...
            if ($store_handler->store_count>1) { // ignore store permission settings
                if(_SYSTEM_GROUP_PERMISSIONS=='blacklist'){
        ...
        $query = "SELECT c.categories_id,c.categories_status, cd.categories_name, c.category_custom_link, c.categories_left, c.categories_right FROM  ".TABLE_CATEGORIES." c
        ...
        if($record->RecordCount() > 0){
            while(!$record->EOF){
                ...
                $cat_data['categories_left']=$record->fields['categories_left'];
                $cat_data['categories_right']=$record->fields['categories_right'];
    ...
    {	global $db, $language,$xtLink;
        ...
        if ($data['category_custom_link']==1)
        {
            switch ($data['category_custom_link_type'])
            {
                ...
                case 'product':
                    $p_info = product::getProduct($data['category_custom_link_id']);
                    $link_array = array(
                        'page'=> 'products',
                        'type'=> 'product',
     ...
     * @return array
     */
    function getCategoryBox ($catID = 0, $nested = false, $maxlevel = 0, $filter='') {
        global $xtPlugin;
        ...
        $tree = $nested_set->getTree(true, $maxlevel);
    ...
    function getCategoryListing ($catID = '',$direct_children_only=0) {
        ...
        if(!empty($catID)) {
            $categoryPath = $this->getPath($catID);
            array_unshift($categoryPath, 0);

            $nested_set->setNestedLevel($categoryPath);
        }

        $tree = $nested_set->getTree();
        $categories = $nested_set->buildHierarchy($tree, $catID, false, array(), $direct_children_only);
     ...
     * @return array
     */
    function getChildCategories ($catID = 0, $level = 0, $nested = false, $buildHierarchy = true) {
        $nested_set = $this->getNestedSet();
        $tree = $nested_set->getTree(true, $level);
        if(!$buildHierarchy) return $tree;
    ...
    function removeCategory($categories_id) {
        ...
        $query = "SELECT * FROM " . TABLE_CATEGORIES . " WHERE categories_id = ?";
        $record = $db->Execute($query, $categories_id);
        if($record->RecordCount() == 0){
            return;
        }
    ...
    function _getParams() {
        ...
        $header['start_page_category'] = array('type' => 'status');
        $header['category_custom_link'] = array('type' => 'hidden');
        $header['categories_level'] = array('type' => 'hidden');
    ...
    function _get($catID = 0) {
        ...
        if ($res==1)
        {
            ...
            echo "addTab('adminHandler.php?load_section=custom_link&edit_id=".$org_cat."','".__text('TEXT_EDIT_CUSTOM_LINK')."');";
            echo '</script>';
        }
    ...
    /**
     * clear smarty category cache files
     */
    function _clearCategoryCache() {
    	
    	if (SMARTY_USE_CACHE!='true') return;
    	
    	$dir = _SRV_WEBROOT.'cache/';
    	$FilesArray = array();
    	if ($handle = opendir($dir)) {
    		while (false !== ($file = readdir($handle))) {
    			if (strpos($file,'.html.php') !== false && strpos($file,'cat^')!== false) {
    				$FilesArray[] =  $file;
    			}
    		}
    		closedir($handle);
    ...
    function _set($data, $set_type = 'edit'){
        ...
        if ($set_type=='new') {
            ...
            $_level = $nested_set->getCategoryParentLevel($data['parent_id']);
            $_level++;
            $data['categories_level'] = $_level;
        }
        ...
        foreach ($stores as $store) {
            foreach ($language->_getLanguageList() as $key => $val) {
                ...
                ($plugin_code = $xtPlugin->PluginCode('class.category.php:_set_seo')) ? eval($plugin_code) : false;
