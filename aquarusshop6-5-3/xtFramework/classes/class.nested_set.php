...
class nested_set extends getCategorySQL_query {
	...
	function getSQL_query($additionalColumns = '', $filter_type ='string') {
		...
		if (USER_POSITION =='admin') {
            $sql = preg_replace("/c.categories_status = '1'[\s]+and/", '', $sql);
		}
	 ...
	 * @return array
	 */
	public function getTree($cached = true, $maxLevel = false)
	{
		global $xtPlugin;
		...
		$this->a_sql_sort = 'c.sort_order, cd.categories_name';
		if((int)$maxLevel)
        {
            $this->setSQL_WHERE(' and c.categories_level <= '.(int)$maxLevel.' ');
        }
	...
	public function buildCustomLinkURL($category){
		...
		switch($category['category_custom_link_type']){
			...
			case 'content':
				...
				// class is category but pages need to be called categorie
				if($category['category_custom_link_type'] == 'category')
				{
					$category['category_custom_link_type'] = 'categorie';
				}
