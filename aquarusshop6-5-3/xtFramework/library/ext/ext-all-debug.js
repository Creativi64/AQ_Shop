...
Ext.Panel = Ext.extend(Ext.Container, {
		...
		addTool: function () {
			...
			if (!this.toolTemplate) {
				var F = new Ext.Template("<i class=\"x-tool x-tool-{id}\"></i>");
...
Ext.Button = Ext.extend(Ext.Component, {
		...
		onRender: function (C, A) {
			if (!this.template) {
				if (!Ext.Button.buttonTemplate) {
					Ext.Button.buttonTemplate = new Ext.Template("<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"\"><tbody><tr>", "<td><button class=\"btn btn-block btn-default btn-sm x-btn-over\" type=\"{1}\"><span class=\"xt-btn-text\">{0}</span></button></td>", "</tr></tbody></table>")
				}
				this.template = Ext.Button.buttonTemplate
			}
			var B,
			E = [this.text || "&#160;", this.type];
			if (A) {
		...
		initButtonEl: function (B, C) {
			this.el = B;
			B.addClass(""); /*if(this.icon){C.setStyle("background-image","url("+this.icon+")")}*/ if (this.iconCls) {
		...
		setText: function (A) {
			...
			if (this.el) {
				var addsel = '';
				if(this.menu) addsel = "tr.x-btn-with-menu ";
				this.el.child(addsel + this.buttonSelector).update(A)
			}
			this.autoWidth()
		},
		getText: function () {
...
(function () {
	...
	Ext.extend(A, Ext.BoxComponent, {
		...
		nextBlock: function () {
			...
			B.setAttribute("class","x-toolbar-editor-element");
...
Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {
		...
		doClear: function () {
			...
			for (var i = 0; i < records.length; i++) {
				var rec = records[i];
				if (rec.get('selectedItem'))
					rec.set('selectedItem', false);
			}
		},
		onClick: function (E) {
			var B = this.store;
...
Ext.extend(Ext.menu.Item, Ext.menu.BaseItem, {
	...
	onRender: function (B, A) {
		...
		C.className = this.itemCls + (this.menu ? " x-menu-item-arrow" : "") + (this.cls ? " " + this.cls : "")+ " "+ this.iconCls;
...
Ext.form.NumberField = Ext.extend(Ext.form.TextField, {
		fieldClass: "x-form-field x-form-num-field form-control",
		allowDecimals: true,
		decimalSeparator: ".",
		decimalPrecision: 2,
		allowNegative: true,
...
Ext.form.Radio = Ext.extend(Ext.form.Checkbox, {
		...
		setValue: function (A) {
			if (typeof A == "boolean") {
				...
				var B = this.el.up("form").child("input[name=" + this.el.dom.name + "][value=" + A + "]", true);
				if (B) {
					B.checked = true
				}
			}
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {
		...
		createToolbar: function (C) {
			...
			var btnTplBak = Ext.Button.buttonTemplate;
            ...
            Ext.Button.buttonTemplate = btnTplBak;
        },
...
Ext.grid.EditorGridPanel = Ext.extend(Ext.grid.GridPanel, {
		...
		preEditValue: function (A, B) {
			return this.autoEncode && typeof value == "string" ? Ext.util.Format.htmlDecode(A.data[B]) : A.data[B]
		},
