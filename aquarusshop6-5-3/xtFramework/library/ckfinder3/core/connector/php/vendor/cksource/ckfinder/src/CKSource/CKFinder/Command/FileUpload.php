...
use Symfony\Component\HttpFoundation\File\UploadedFile as SymUploadedFile;
...
class FileUpload extends CommandAbstract
{
    ...
    public function execute(Request $request, WorkingFolder $workingFolder, EventDispatcher $dispatcher, Config $config, CacheManager $cache, ThumbnailRepository $thumbsRepository)
    {
        ...
        // xt anpassung
        $uploadFile = new SymUploadedFile(
            $upload["tmp_name"],
            $upload["name"],
            $upload["type"],
            $upload["size"],
        ...
        // xt anpassung
        $uploadedFile = new UploadedFile($uploadFile, $this->app);
