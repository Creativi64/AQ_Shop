...
class Update60xTo61x extends ExecutableScript {
    ...
    */
    public function doUpdate(WizardPage $page)
    {
        ...
        $this->addControllerCommand(ControllerCommand::factory()
            ->setAction(function(ControllerCommand $command) use ($idb, $store_handler) {
                ...
                try{
                    ...
                    foreach ($stores as $store)
                    {
                        ...
                        $_key_exists = $idb->GetOne('SELECT 1 FROM '.$table." WHERE config_key = '_STORE_GOTO_CART_DIRECTLY'");
                        if(!$_key_exists)
                        {
                            $idb->Execute('INSERT INTO ' . $table . " (config_key, config_value, group_id, sort_order, type) VALUES ('_STORE_GOTO_CART_DIRECTLY', '1', '18', '35', 'status')");
                        }
                    }
                }catch (Exception $e){
            ...
            ->setDescription(TEXT_UPDATING . " DB _config_x _STORE_GOTO_CART_DIRECTLY<br/>")
            ->setAbortOnError(true)
        );

        // _STORE_STAY_ON_PAGE_AFTER_CUSTOMER_LOGIN
        $this->addControllerCommand(ControllerCommand::factory()
            ->setAction(function(ControllerCommand $command) use ($idb, $store_handler) {
                $idb = Wizard::getInstance()->getDatabaseObject();
                try{
                    ...
                    {
                        $table = DB_PREFIX.'_config_'.$store['id'];
                        $_key_exists = $idb->GetOne('SELECT 1 FROM '.$table." WHERE config_key = '_STORE_STAY_ON_PAGE_AFTER_CUSTOMER_LOGIN'");
                        if(!$_key_exists)
                        {
                            $idb->Execute('INSERT INTO ' . $table . " (config_key, config_value, group_id, sort_order, type) VALUES ('_STORE_STAY_ON_PAGE_AFTER_CUSTOMER_LOGIN', '1', '18', '40', 'status')");
                        }
                    }
                }catch (Exception $e){
                    $command->setErrorMessage("<span class='msg-error'>".TEXT_QUERY_FAILED." " . $e->getMessage() . "</span><br/>");
            ...
            ->setDescription(TEXT_UPDATING . " DB _config_x _STORE_STAY_ON_PAGE_AFTER_CUSTOMER_LOGIN<br/>")
            ->setAbortOnError(true)
        );

        // move _SYSTEM_ADMIN_RIGHTS
        $this->addControllerCommand(ControllerCommand::factory()
            ->setAction(function(ControllerCommand $command) use ($idb, $store_handler, $install_prefix) {
                $idb = Wizard::getInstance()->getDatabaseObject();
                try{
                    $idb->Execute('UPDATE ' . $install_prefix.'config' . " SET config_value='db', group_id=17, type='hidden' WHERE config_key = '_SYSTEM_ADMIN_RIGHTS'");
                }catch (Exception $e){
                    $command->setErrorMessage("<span class='msg-error'>".TEXT_QUERY_FAILED." " . $e->getMessage() . "</span><br/>");
                    return false;
                }
            ...
            ->setDescription(TEXT_UPDATING . " DB _config _SYSTEM_ADMIN_RIGHTS<br/>")
            ->setAbortOnError(true)
        );

        // move _SYSTEM_AUTOLOAD
        $this->addControllerCommand(ControllerCommand::factory()
            ->setAction(function(ControllerCommand $command) use ($idb, $store_handler, $install_prefix) {
                $idb = Wizard::getInstance()->getDatabaseObject();
                try{
                    $idb->Execute('UPDATE ' . $install_prefix.'config' . " SET group_id=25 WHERE config_key = '_SYSTEM_AUTOLOAD'");
                }catch (Exception $e){
                    $command->setErrorMessage("<span class='msg-error'>".TEXT_QUERY_FAILED." " . $e->getMessage() . "</span><br/>");
                    return false;
                }
            ...
            ->setDescription(TEXT_UPDATING . " DB _config _SYSTEM_AUTOLOAD<br/>")
            ->setAbortOnError(true)
        );


        // update config_group 'performance' set hidden    TEXT_CONF_PERFORMANCE
        $this->addControllerCommand(ControllerCommand::factory()
            ->setAction(function(ControllerCommand $command) use ($idb, $store_handler, $install_prefix) {
                $idb = Wizard::getInstance()->getDatabaseObject();
                try{
                    $idb->Execute('UPDATE ' . $install_prefix.'config_group' . " SET visible=0 WHERE group_title = 'TEXT_CONF_PERFORMANCE'");
                }catch (Exception $e){
                    $command->setErrorMessage("<span class='msg-error'>".TEXT_QUERY_FAILED." " . $e->getMessage() . "</span><br/>");
                    return false;
                }
            ...
            ->setDescription(TEXT_UPDATING . " DB _config_group performance<br/>")
            ->setAbortOnError(true)
        );
...
Wizard::getInstance()->registerScript(new Update60xTo61x());
