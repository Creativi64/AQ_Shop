...
class plugin
{
     ...
     * Example: "ewadventurythumb"
     */
    const IMAGE_TYPE_PREFIX = 'ew-adventury-';

    /**
     * Content Block Prefix
     * Example: "ew_adventury_teaser"
     */
    const CONTENT_BLOCK_PREFIX = 'ew_adventury_';

    /**
     * Name of current plugin
     */
    const PLUGIN_NAME = 'ew_adventury_plugin';
    ...
    {
        return self::check_conf('CONFIG_EW_ADVENTURY_PLUGIN_COMPACT_TEASER');
    }
     ...
     * @example Usage in smarty: {$smarty.const.EW_ADVENTURY_SHOP_ID}
     */
    public static function setShopIdConstant()
    {
        defined('EW_ADVENTURY_SHOP_ID') or define('EW_ADVENTURY_SHOP_ID', self::getShopId());
    }
    ...
    {
        return (defined('CONFIG_EW_ADVENTURY_PLUGIN_PRIMARY_COLOR') && self::isHexColorString(CONFIG_EW_ADVENTURY_PLUGIN_PRIMARY_COLOR)) ? CONFIG_EW_ADVENTURY_PLUGIN_PRIMARY_COLOR : null;
    }
            ...
            if (!isset($container[$parent])) {
                if ($parent !== 0) {
                    ...
                    $container[$parent] = [
                        ...
                        'categories_name' => defined('TEXT_EW_ADVENTURY_NAVBAR_MOBILE_TITLE') ? TEXT_EW_ADVENTURY_NAVBAR_MOBILE_TITLE : 'Menu',
    ...
    {
        return (defined('CONFIG_EW_ADVENTURY_PLUGIN_CAT_MAXLEVEL') && (int)CONFIG_EW_ADVENTURY_PLUGIN_CAT_MAXLEVEL > $default) ? (int)CONFIG_EW_ADVENTURY_PLUGIN_CAT_MAXLEVEL : $default;
    }
    ...
    {
        if (!defined('CONFIG_EW_ADVENTURY_PLUGIN_DEBUG_MODE') || empty(CONFIG_EW_ADVENTURY_PLUGIN_DEBUG_MODE))
            return false;

        if (trim(CONFIG_EW_ADVENTURY_PLUGIN_DEBUG_MODE) == '*')
            return true;

        $ipArray = array();
        foreach (explode(',', CONFIG_EW_ADVENTURY_PLUGIN_DEBUG_MODE) as $ip) {
    ...
    /**
     * Manages the eyecatcher display
     *
     * @return boolean
     */
    public static function show_eyecatcher()
    {
        return self::is_index();
    }
    ...
    public function getConfig($key)
    {
        if (!isset($this->_configurationArray[$key]) || empty($this->_configurationArray[$key])) {
            $filePath = EW_ADVENTURY_PLUGIN_CONFIG_DIR . DIRECTORY_SEPARATOR . (string)$key . '.xml';
    ...
    public static function getSliderId($overwrite = 0, $fallback = 1)
    {
        ...
        $config = defined('CONFIG_EW_ADVENTURY_PLUGIN_TEASER_SLIDER_ID') ? (int)CONFIG_EW_ADVENTURY_PLUGIN_TEASER_SLIDER_ID : 0;
    ...
    {
        if (!$forceOldMethod && version_compare(_SYSTEM_VERSION, '5.0.0', '>=')) {
            return $this->buildCategoriesArrayNew($max_level, $cid, $level, $cache, $recursive);
        }

        return $this->old_build_categories_array($max_level, $cid, $level, $cache, $recursive);
    }
     ...
     * @return array
     */
    protected function old_build_categories_array($max_level = 5, $cid = 0, $level = 0, $cache = true, $recursive = false)
    {
        global $category, $current_category_id;
        ...
        if ($max_level > 0) {
            ...
            if (is_array($data) && !empty($data)) {
                foreach ($data as $k => $i) {
                    if (($level + 1) == $i['level']) {
                        $build_array[$k] = $i;
                        $child = ($recursive === true || in_array($i['categories_id'], $category->level)) ? $this->old_build_categories_array($max_level - 1, $i['categories_id'], $level + 1, $cache, $recursive) : null;
    ...
    public function registerAssets()
    {
        ...
        define('EW_ADVENTURY_TEMPLATE_RELATIVE_DIR', _SRV_WEB_TEMPLATES . _STORE_TEMPLATE);
        define('EW_ADVENTURY_TEMPLATE_ROOT_DIR', _SRV_WEBROOT . EW_ADVENTURY_TEMPLATE_RELATIVE_DIR);
        define('EW_ADVENTURY_PLUGIN_RELATIVE_DIR',  _SRV_WEB_PLUGINS . self::PLUGIN_NAME);
        ...
        $files = array(
            ...
            '../../' . EW_ADVENTURY_TEMPLATE_RELATIVE_DIR . '/css/' . self::get_unique_shop_string('stylesheet_', '.css'),
            '../../' . EW_ADVENTURY_TEMPLATE_RELATIVE_DIR . '/javascript/' . self::get_unique_shop_string('script_', '.js'),
        );
        ...
        if (($lessConfig = $this->getConfig('lessCss')) !== null) {
            if ($lessConfig['config']['parseLess'] == 'true') {
                ...
                if (self::isDebugMode() || !file_exists($compileToFile)) {
                    ...
                    $less->parseString = ($lessConfig['config']['parsePluginSettings'] == 'true') ? self::buildLessColorFromConfig('@brand-primary', 'CONFIG_EW_ADVENTURY_PLUGIN_PRIMARY_COLOR') : null;
                    $less->forceUpdate = ($lessConfig['config']['parsePluginSettings'] == 'true') ? self::checkLessStringConfigUpdateTime($compileToFile, 'CONFIG_EW_ADVENTURY_PLUGIN_PRIMARY_COLOR') : false;
                    ...
                    $less->init(array_merge($lessFiles, array(
                        array(
                            'input'     => EW_ADVENTURY_TEMPLATE_ROOT_DIR . '/css/' . self::get_unique_shop_string('template_', '.less'),
                            'webFolder' => '../' . EW_ADVENTURY_TEMPLATE_RELATIVE_DIR . '/css/',
                        ),
                    )), $compileToFile);
                }
            }
        ...
        if (($assetsNoMinify = $this->getConfig('assetsNoMinify')) !== null &&
            ...
            foreach ($assetsNoMinify['file'] as $k => $f) {
                $filePath = EW_ADVENTURY_PLUGIN_ROOT_DIR . DIRECTORY_SEPARATOR . $f;
                if (@is_file($filePath)) {
        ...
        foreach ($files as $k => $f) {
            if (@is_file(EW_ADVENTURY_PLUGIN_ROOT_DIR . DIRECTORY_SEPARATOR . $f)) {
                $ext = pathinfo($f, PATHINFO_EXTENSION);
                if (isset($xtMinify) && is_object($xtMinify)) {
                    switch ($ext) {
                        case 'css':
                            $xtMinify->add_resource(EW_ADVENTURY_PLUGIN_RELATIVE_DIR . '/' . $f, $k);
                            break;
                        case 'js':
                            $xtMinify->add_resource(EW_ADVENTURY_PLUGIN_RELATIVE_DIR . '/' . $f, $k*2+5, 'footer');
                            break;
                    }
                } else {
                    switch ($ext) {
    ...
    public static function resolveStringToPath($path)
    {
        return @str_replace(
            array(
                'EW_ADVENTURY_PLUGIN_ROOT_DIR',
                'EW_ADVENTURY_PLUGIN_RELATIVE_DIR',
                'EW_ADVENTURY_TEMPLATE_ROOT_DIR',
                'EW_ADVENTURY_TEMPLATE_RELATIVE_DIR',
            ),
            array(
                EW_ADVENTURY_PLUGIN_ROOT_DIR,
                EW_ADVENTURY_PLUGIN_RELATIVE_DIR,
    ...
    {
        return self::check_conf('CONFIG_EW_ADVENTURY_PLUGIN_LESS_SOURCE_MAP');
    }
