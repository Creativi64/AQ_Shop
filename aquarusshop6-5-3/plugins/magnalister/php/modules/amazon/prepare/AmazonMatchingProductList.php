...
class AmazonMatchingProductList extends MLProductListAmazonAbstract {
	...
	protected function buildQuery(){
		...
		// Get the applytable keys first (no subquery, cos it takes too long if the table is big)
		$aNotIn = MagnaDB::gi()->fetchArray("SELECT DISTINCT $sKeyType
					FROM ".TABLE_MAGNA_AMAZON_APPLY."
					WHERE mpID = ".$this->aMagnaSession['mpID']);
		$sNotIn = '';
		foreach ($aNotIn as $row) {
			$tmpId = trim(array_pop($row));
			if (empty($tmpId)) continue;
			$sNotIn .= ",'".MagnaDB::gi()->escape($tmpId)."'";
		}
		$sNotIn = trim($sNotIn, ',');
		if (!empty($sNotIn)) {
			parent::buildQuery()->oQuery->where(
				'p.'.$sKeyType.' NOT IN ('.$sNotIn.')'
			);
		} else {
			parent::buildQuery();
		}
