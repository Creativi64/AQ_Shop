...
class AmazonSummaryView extends SimpleSummaryView {
			...
			try {
				...
				if (!empty($result['DATA'])) {
					foreach ($result['DATA'] as $item) {
						...
						if ($item['LowestPrice']['CurrencyCode'] != getCurrencyFromMarketplace($this->_magnasession['mpID'])) continue;
	...
	protected function getAdditionalItemCells($key, $dbRow) {
		if (getDBConfigValue('general.keytype', '0') == 'artNr') {
			$dbRow['lowestprice'] = MagnaDB::gi()->fetchOne('
				SELECT lowestprice 
				  FROM '.TABLE_MAGNA_AMAZON_PROPERTIES.' 
				 WHERE products_model=\''.MagnaDB::gi()->escape($dbRow['products_model']).'\'
				       AND asin<>\'\'
				       AND mpID=\''.$this->_magnasession['mpID'].'\'
			');
		} else {
			$dbRow['lowestprice'] = MagnaDB::gi()->fetchOne('
				SELECT lowestprice 
				  FROM '.TABLE_MAGNA_AMAZON_PROPERTIES.'
				 WHERE products_id=\''.$dbRow['products_id'].'\'
				       AND asin<>\'\'
				       AND mpID=\''.$this->_magnasession['mpID'].'\'
			');
		}
		...
		if (array_key_exists('lowestprice', $dbRow) && ((float)$dbRow['lowestprice'] > 0)) {
		...
		$html = '
				...
				<td><table class="nostyle"><tbody>
						<tr><td>'.ML_LABEL_NEW.':&nbsp;</td><td>'.$this->simplePrice->setPrice($this->selection[$dbRow['products_id']]['price'])->formatWOCurrency().'
