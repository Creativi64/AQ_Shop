...
class MLProductListDependencyPriceministerMatchingFormAction extends MLProductListDependency {
	public function getActionBottomLeftTemplate(){
		return 'priceministermatchingformleft';
	}
	
	public function getActionBottomRightTemplate(){
		return 'priceministermatchingformright';
	}
	
	public function getHeaderTemplate() {
		return 'priceministermatchingform';
	}
	...
	protected function unmatching(){
		...
		if (!empty($pIDs)) {
			...
			if ( MLProduct::gi()->hasMasterItems()) {
				// delete dummy master, if it don't have matched variant
				...
				", false), true) as $iMaster) {
					$aMaster = MLProduct::gi()->setLanguage(getDBConfigValue('priceminister.lang', $this->getMagnaSession('mpID'), $_SESSION['magna']['selected_language']))->getProductById($iMaster);
					$aVariants = array();
					foreach ($aMaster['Variations'] as $aVariant) {
						$aVariants[] = $aVariant[(($sKeyType == 'products_model') ? 'MarketplaceSku' : 'VariationId')];
					}
					if (   empty($aVariants)
						|| 0 == MagnaDb::gi()->fetchOne("
									SELECT COUNT(".$sKeyType.")
									  FROM ".TABLE_MAGNA_PRICEMINISTER_PREPARE."
									 WHERE     mpID = '".$this->getMagnaSession('mpID')."'
									       AND ".$sKeyType." IN('".implode("', '", $aVariants)."')
									       AND PrepareType = 'Match'
								")
					) {
						MagnaDB::gi()->delete(TABLE_MAGNA_PRICEMINISTER_PREPARE, array(
							'mpID' => $this->getMagnaSession('mpID'),
