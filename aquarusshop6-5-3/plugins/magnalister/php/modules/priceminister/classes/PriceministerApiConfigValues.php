...
class PriceministerApiConfigValues extends MagnaCompatibleApiConfigValues
{
    ...
    public function getVariantConfigurationDefinition($which, $onlyAdvert = false)
    {
        $data = array(
            'DATA' => array(
                'CategoryID' => $which,
        ...
        if ($onlyAdvert){
            $data['DATA']['OnlyAdvert'] = 'true';
        }

        $data = $this->fetchDataFromApi('GetCategoryDetails', $data);
        return $data;
    }

    public function getSubcategoryAttributesForCategory($categoriId)
    {
       return $this->fetchDataFromApi('GetCategoryAttributes', array(
            'DATA' => array(
