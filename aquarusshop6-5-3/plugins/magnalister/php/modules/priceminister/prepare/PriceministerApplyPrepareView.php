...
class PriceministerApplyPrepareView extends MagnaCompatibleBase {
    ...
    public function process() {
        ...
        $attributeMatchingTableHtml = '
            ...
            <tbody id="tbodyDynamicMatchingHeadline" style="display:none;">
                <tr class="headline">
                    <td colspan="1"><h4>' . str_replace('%marketplace%', 'PriceMinister', ML_GENERAL_VARMATCH_MP_ATTRIBUTE) . '</h4></td>
        ...
        <script type="text/javascript" src="js/marketplaces/priceminister/variation_matching.js?<?php echo CLIENT_BUILD_VERSION?>"></script>
    ...
    public function renderAjax() {
        if (isset($_GET['where']) && ($_GET['where'] == 'catMatchView')) {
            ...
            return json_encode(PriceministerHelper::gi()->getMPVariations($select, $productModel, true));
    ...
    protected function getSelection() {
        ...
        $dbNewSelectionQuery = '
			...
			LEFT JOIN ' . TABLE_PRODUCTS_DESCRIPTION . ' pd ON pd.products_id = p.products_id AND pd.language_code = "' .
            $sLanguageCode . '"
