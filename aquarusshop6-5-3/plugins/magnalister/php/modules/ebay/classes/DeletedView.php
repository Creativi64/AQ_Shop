 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: DeletedView.php 167 2013-02-08 12:00:00Z tim.neumann $
 *
...
class DeletedView {
    ...
    private function getShopDataForItems() {
        ...
        if ('artNr' == getDBConfigValue('general.keytype', '0')) {
            $ShopDataForSimpleItems = MagnaDB::gi()->fetchArray('
				SELECT DISTINCT p.products_model SKU, p.products_id products_id, 
								CAST(p.products_quantity AS SIGNED) ShopQuantity, p.products_price ShopPrice, pd.products_name ShopTitle 
        ...
        /*$ShopDataForVariationItems = MagnaDB::gi()->fetchArray('SELECT DISTINCT v.variation_products_model SKU, v.products_id products_id,
                                        REPLACE(REPLACE(TRIM(TRAILING \'|\' FROM v.variation_attributes_text),\',\',\':\'),\'|\',\', \') ShopVarText,
                                        ...
                                        AND v.variation_products_model IN ('.$SKUlist.')');*/
