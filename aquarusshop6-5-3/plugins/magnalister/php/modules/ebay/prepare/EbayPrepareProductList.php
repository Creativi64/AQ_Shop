...
class EbayPrepareProductList extends MLProductListEbayAbstract {
	...
	protected function getPreparedStatusIndicator($aRow){
		...
		if ($sVerified === null) {
			return html_image(DIR_MAGNALISTER_IMAGES . 'status/grey_dot.png', ML_EBAY_PRODUCT_MATCHED_NO, 9, 9);
		}
		if ('OK' != $sVerified) {
			if ('EMPTY' == $sVerified) {
				return html_image(DIR_MAGNALISTER_IMAGES . 'status/grey_dot.png', ML_EBAY_PRODUCT_PREPARED_FAULTY_BUT_MP, 9, 9);
			} else {
				return html_image(DIR_MAGNALISTER_IMAGES . 'status/red_dot.png', ML_EBAY_PRODUCT_PREPARED_FAULTY, 9, 9);
			}
		}else{
			return html_image(DIR_MAGNALISTER_IMAGES . 'status/green_dot.png', ML_EBAY_PRODUCT_PREPARED_OK, 9, 9);
		}
	}
