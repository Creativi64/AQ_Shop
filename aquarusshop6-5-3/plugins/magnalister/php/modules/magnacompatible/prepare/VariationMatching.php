...
abstract class VariationMatching
{
    ...
    protected function saveMatching($redirect = true)
    {
        if (isset($_POST['ml']['match'])) {
            ...
            if (!isset($_POST['Action']) || $_POST['Action'] !== 'ResetMatching') {
                $oClass = $this->getAttributesMatchingHelper();
                if (get_class($oClass) === 'AttributesMatchingHelper2' || get_class($oClass) === 'AmazonHelper') {
                    /* @var $oClass AttributesMatchingHelper2 */
                    $this->aErrors = array_merge($this->aErrors, $oClass->saveMatching($sIdentifier, $matching, $redirect, false, false, null, $sCustomIdentifier));
                } else {
                    $this->aErrors = array_merge($this->aErrors, $oClass->saveMatching($sIdentifier, $matching, $redirect, false, $sCustomIdentifier));
                }
            }
    ...
    public function renderAjax() {
        ...
        if (isset($_GET['where']) && ($_GET['where'] == 'catMatchView')) {
            ...
            if (get_class($oClass) === 'AttributesMatchingHelper2' || get_class($oClass) === 'AmazonHelper') {
                /* @var $oClass AttributesMatchingHelper2 */
                $data = $oClass->getMPVariations($select, false, true, null, $customIdentifier);
            } else {
                $data = $oClass->getMPVariations($select, false, true, $customIdentifier);
            }
            ...
            if (get_class($oClass) === 'AttributesMatchingHelper2' || get_class($oClass) === 'AmazonHelper') {
                /* @var $oClass AttributesMatchingHelper2 */
                $data = $oClass->getMPVariations($params['PrimaryCategory'], false, true, null, $params['CustomIdentifier']);
            } else {
                $data = $oClass->getMPVariations($params['PrimaryCategory'], false, true, $params['CustomIdentifier']);
            }
            $data['notice'] = array();
            foreach ($this->aErrors as $error) {
