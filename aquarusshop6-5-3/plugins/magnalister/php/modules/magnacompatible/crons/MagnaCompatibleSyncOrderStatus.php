...
class MagnaCompatibleSyncOrderStatus extends MagnaCompatibleCronBase {
	...
	protected function getCarrier($orderId) {
        /* {Hook} "MagnaCompatibleSyncOrderStatus_getCarrier": Enables you to replace the Carrier identification by your own functionality
           Variables that can be used:
           <ul><li>$orderId: The ID of the current order (read only)</li>
               <li>$sCarrier: Your calculated Carrier</li>
           </ul>
        */
        if (($hp = magnaContribVerify('MagnaCompatibleSyncOrderStatus_getCarrier', 1)) !== false) {
            require($hp);
	...
	protected function updateUnprocessed() {
        ...
        foreach ($this->unprocessed as $sOrderId => $sOrderStatus) {
