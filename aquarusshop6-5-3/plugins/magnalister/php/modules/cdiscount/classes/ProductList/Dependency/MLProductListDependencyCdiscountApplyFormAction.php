...
class MLProductListDependencyCdiscountApplyFormAction extends MLProductListDependency {
	public function getActionBottomLeftTemplate(){
		return 'cdiscountapplyformleft';
	}
	
	public function getActionBottomRightTemplate(){
		return 'cdiscountapplyformright';
	}
	...
	protected function removeApply() {
		...
		if (!empty($pIDs)) {
			foreach ($pIDs as $pID) {
				$aProduct = MLProduct::gi()->setLanguage(getDBConfigValue('cdiscount.lang', $this->getMagnaSession('mpID'), $_SESSION['magna']['selected_language']))->getProductById($pID);
				...
				MagnaDB::gi()->delete(TABLE_MAGNA_CDISCOUNT_PREPARE, $where, $sAdd);
	...
	protected function resetApply() {
		...
		if (!empty($pIDs)) {
			if (getDBConfigValue('general.keytype', '0') == 'artNr') {
				$aProducts = MagnaDB::gi()->fetchArray('
					SELECT aa.products_id AS PID,
					       ...
					       aa.*
					  FROM '.TABLE_MAGNA_CDISCOUNT_PREPARE.' aa
			...
			foreach ($aProducts as $aRow) {				
				...
				MagnaDB::gi()->update(TABLE_MAGNA_CDISCOUNT_PREPARE, array (
	...
	private function getArticleFromShop($iProductId) {
		...
		$lang = mlGetLanguageCodeFromID($langID);
		...
		$product['Subtitle'] = $prod[0]['Subtitle'];
