...
class EtsyPrepare extends MagnaCompatibleBase {
...
#echo print_m($_GET, '$_GET');
	...
	if (    (isset($_GET['mode']) && $_GET['mode'] == 'prepare')
        ...
        /*if (isset($_POST['request'])
            ...
            $product = array(
                'Id' => $_POST['productID'],
                'Results' => EtsyHelper::SearchOnEtsy($sSearch, $sType)
            );
...
#echo print_m($sClass, __METHOD__.' '.__LINE__.' $sClass');
        ...
        foreach ($products as $product) {
            $searchResults = EtsyHelper::SearchOnEtsy($product['EAN'], 'EAN');

            if ($searchResults === false
                || (is_array($searchResults) && count($searchResults) === 0)
            ) {
                $searchResults = EtsyHelper::SearchOnEtsy($product['Title'], 'Title');
            }
            ...
            MagnaDB::gi()->insert(TABLE_MAGNA_ETSY_PREPARE, $matchedProduct, true);
            MagnaDB::gi()->delete(TABLE_MAGNA_SELECTION, array(
                'pID' => $product['Id'],
                'mpID' => $this->mpID,
                'selectionname' => 'Match',
	...
	#echo print_m($_POST, '$_POST');
        ...
        $oProductSaver = new EtsyProductSaver($this->resources['session']);
...
#echo print_m($savePrepare, __METHOD__.' '.__LINE__.' savePrepare');

        $oProductSaver->aErrors = array_merge($oProductSaver->aErrors,
            EtsyHelper::gi()->saveMatching($sIdentifier, $matching, $savePrepare, true, $isSinglePrepare, $variationThemeAttributes));
    ...
    protected function deletePrepare() {
        ...
        foreach ($pIDs as $pID) {
            ...
            MagnaDB::gi()->delete(TABLE_MAGNA_ETSY_PREPARE, $where);
            MagnaDB::gi()->delete(TABLE_MAGNA_SELECTION, array(
                'pID' => $pID,
                'mpID' => $this->mpID,
                'selectionname' => $this->prepareSettings['selectionName'],
    ...
    protected function getSelectedProductsCount() {
        $query = '
			...
			LEFT JOIN '.TABLE_MAGNA_ETSY_PREPARE.' p on p.mpID = s.mpID and p.products_id = s.pID
...
#echo "<br />\n".__METHOD__."<br />\n";
        ...
        } else*/if ($this->prepareSettings['selectionName'] === 'varmatch') {
