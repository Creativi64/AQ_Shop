...
if (MLBrowserDetect::gi()->compare('Browser', 'msie', '==')) {
	echo '		<meta http-equiv="x-ua-compatible" content="IE='.((int)MLBrowserDetect::gi()->get('BVersion')).'">'."\n";
		...
		<link rel="stylesheet" type="text/css" href="css/magnalister.css?<?php echo CLIENT_BUILD_VERSION?>" />
        <link rel="stylesheet" type="text/css" href="css/select2/select2.min.css?<?php echo CLIENT_BUILD_VERSION?>" />
        <link rel="stylesheet" type="text/css" href="css/select2/fix-select2.css?<?php echo CLIENT_BUILD_VERSION?>" /><?php
...
'.$_pageCSS.'
        ...
        <script type="text/javascript" src="js/select2/select2.min.js?<?php echo CLIENT_BUILD_VERSION ?>"></script>
        <script type="text/javascript" src="js/select2/i18n/<?php echo $_SESSION['selected_language'] ?>.js?<?php echo CLIENT_BUILD_VERSION ?>"></script>
				...
				$('#globalButtonBox span').on('click', (function() {
					...
					return function (e) {
						...
						if ($(e.target).data('href').search('update') > 0) {
							popupTitle = '<?php echo ML_MESSAGE_BEFORE_XTMC_UPDATE_TITLE ?>';
							popupText  = '<?php echo ML_MESSAGE_BEFORE_XTMC_UPDATE_TEXT ?>';
