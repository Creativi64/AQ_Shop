...
function resizeImage($resource_file, $max_width, $max_height, $destination_file, $compression=80) {
	...
	$src['image'] = function_exists('imagecreatefromstring') ? @imagecreatefromstring(@file_get_contents($resource_file)) : null;

	if (!is_resource($src['image']) && !is_object($src['image'])) { // PHP >= 8.0: resources replaced by GdImage class objects
		unset($src);
		unset($dst);
		return false;
	}
...
function convert2Bytes($val) {
    ...
    // cat val to int for PHP 7.1 notice
    $val = (int)$val;
...
function eecho($str, $print = false, $pre = true) {
	if ($print) {
		if ($pre) {
			echo '<pre>'.$str.'</pre>';
		} else {
...
# #2017100510001153: Umlaut broken in json encoding (backslash stripped). Cannot properly be decoded,
...
function magnaGetLanguageCode($lang) {
	return MagnaDB::gi()->fetchOne('SELECT code FROM '.TABLE_LANGUAGES.' WHERE LOWER(name)=\''.strtolower($lang).'\' LIMIT 1');
}
