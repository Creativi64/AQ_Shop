...
class MagnaRecalcOrdersTotal {
	...
	public function recalcExistingOrderTotalShipping ($iOrderId, $aOrderTotalShipping) {
		$aOrder = MagnaDB::gi()->fetchRow(eecho('
			SELECT o.orders_id, mo.special, mo.platform, mo.mpID
			FROM '.TABLE_ORDERS.' o
			LEFT JOIN '.TABLE_MAGNA_ORDERS.' mo ON o.orders_id = mo.orders_id
			WHERE o.orders_id = \''.(int)$iOrderId.'\'
		', false));
		if (empty($aOrder)) {
			if ($this->_debug) echo __CLASS__.'::'.__METHOD__."(): Order ($iOrderId) not found. Skipping calculation.\n";
			return false;
		} else {
			$aOrderTotalShipping['orders_id'] = $iOrderId;
			$aExistingOrderShippingTotal = MagnaDB::gi()->fetchRow("
				SELECT * FROM ".TABLE_ORDERS_TOTAL." WHERE orders_id = '".$iOrderId."' AND orders_total_key = '".$aOrderTotalShipping['orders_total_key']."'
			");
			if (empty($aExistingOrderShippingTotal)) {
				return MagnaDB::gi()->insert(TABLE_ORDERS_TOTAL, $aOrderTotalShipping);
			} else {// getting calculated (config) value from import-orders
				$oSimplePrice = new SimplePrice();
				$aOrderTotalShipping['orders_total_tax'] = $aExistingOrderShippingTotal['orders_total_tax'];
				$aOrderTotalShipping['orders_total_tax_class'] = $aExistingOrderShippingTotal['orders_total_tax_class'];
				$aOrderTotalShipping['orders_total_price'] = $oSimplePrice
					->setCurrency(MagnaDB::gi()->fetchOne('SELECT currency_code FROM '.TABLE_ORDERS.' WHERE orders_id=\''.$iOrderId.'\''))
					->setPrice($aOrderTotalShipping['orders_total_price'])
					->removeTax($aOrderTotalShipping['orders_total_tax'])
					->getPrice()
				;
				return MagnaDB::gi()->update(
					...
					$aOrderTotalShipping, 
					array (
						'orders_id' => $aOrderTotalShipping['orders_id'],
						'orders_total_key' => $aOrderTotalShipping['orders_total_key'],
					)
				);
			}
		}
