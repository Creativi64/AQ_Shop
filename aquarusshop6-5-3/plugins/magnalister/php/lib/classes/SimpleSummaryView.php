...
class SimpleSummaryView {
	...
	public function __construct($settings = array()) {
		...
		$this->iShopMandant = getDBConfigValue($this->_magnasession['currentPlatform'].'.shopmandant', $this->_magnasession['mpID'], null);
	...
	protected function setupQuery($addFields = '', $addFrom = '', $addWhere = '') {
		$this->dbQuery = '
		    SELECT DISTINCT p.products_id, p.products_price, p.products_image, p.products_quantity, p.products_tax_class_id,
		           p.products_model,
		           pd.products_name
		           '.(($addFields != '') ? (', '.$addFields.' ') : ' ').'
		...
		INNER JOIN '.TABLE_PRODUCTS_DESCRIPTION.' pd ON (
						...
						'.(((null !== $this->iShopMandant) && (MagnaDB::gi()->columnExistsInTable('products_store_id', TABLE_PRODUCTS_DESCRIPTION)))? ' AND pd.products_store_id = '.$this->iShopMandant.' ' : '').'
		           )
		'.$addFrom.'
...
<?php
	...
	public function renderSelection() {
		...
		foreach($selectedProductsDetails as $key => $prod) {
			...
			//masoud commented if (getDBConfigValue(array($this->marketplace.'.price.usespecialoffer', 'val'), $this->mpID, false)) {
			...
			$html .= '
				<tr class="'.(($isOdd = !$isOdd) ? 'odd' : 'even').'">
					...
					<td><ul><li>'.str_replace('<br />', '</li><li>', renderCategoryPath($prod['products_id'], 'product')).'</li></ul></td>
					<td>
