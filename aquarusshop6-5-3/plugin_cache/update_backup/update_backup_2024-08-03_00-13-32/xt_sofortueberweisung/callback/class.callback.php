...
class callback_xt_sofortueberweisung extends callback {
	...
	function process() {
		...
		$hash_array = array(
 			...
 			'sender_holder' => html_entity_decode($this->data['sender_holder']),
			...
			'project_password' => constant('XT_SOFORTUEBERWEISUNG_PROJECT_PASSWORD')
		);
	...
	function _checkAmount() {
		...
		if ($this->data['currency_id']!=$currency) {
			$log_data = array();
            $log_data['class'] = 'sofort check failed';
	...
	function _checkHashSignature()
    {
		if ($this->hash_calulated!=$this->data['hash'] or !isset($this->data['hash'])) {
			...
			$log_data['module'] = 'xt_sofortueberweisung';
			$log_data['orders_id'] = $this->orders_id;
			$log_data['error_msg'] = 'hash check failed ('.$this->hash_calulated.' calculated) vs ('.$this->data['hash'].' given)';
			$log_data['error_data'] = serialize($this->data);
			$log_data['callback_data'] = $_POST;
			$this->_addLogEntry($log_data);
			return false;
	...
	function _checkOrder() {
		...
		if ($error) {
			...
			$log_data['module'] = 'xt_sofortueberweisung';
			$log_data['orders_id'] = $this->orders_id;
			$log_data['error_msg'] = 'orders_id/customers_id check failed ('.$this->orders_id.'/'.$this->customers_id.')';
			$log_data['error_data'] = serialize($this->data);
			$log_data['callback_data'] = $_POST;
			$this->_addLogEntry($log_data);
		}
	...
	function _setStatus($status = 'success') {
		...
		switch ($status) {
			...
			case 'success' :
				$new_status = constant('XT_SOFORTUEBERWEISUNG_STATUS_SUCCESS');
				$log_data['orders_id'] = $this->orders_id;
				$log_data['class'] = 'success';
				$log_data['module'] = 'xt_sofortueberweisung';
				$log_data['transaction_id'] = $this->data['transaction'];
                //$log_data['callback_data'] = $_POST;
				$log_data['callback_data'] = array('status'=>$status, 'message'=>'OK','error'=>'200','transaction_id'=>$this->data['transaction'],'shop_id'=>$store_handler->shop_id);
                break;

				// error
			default:
				$new_status = constant('XT_SOFORTUEBERWEISUNG_STATUS_ERROR');
				$log_data['orders_id'] = $this->orders_id;
				$log_data['class'] = 'error';
				$log_data['module'] = 'xt_sofortueberweisung';
				$log_data['transaction_id'] = $this->data['transaction'];
                //$log_data['callback_data'] = $_POST;
				$log_data['callback_data'] = array('status'=>$status, 'message'=>'FAILED','error'=>'999','transaction_id'=>$this->data['transaction'],'shop_id'=>$store_handler->shop_id);
                break;
		}
        $callback_id = $this->_addLogEntry($log_data);

        // update order status
		$this->_updateOrderStatus($new_status,constant('XT_SOFORTUEBERWEISUNG_STATUS_NOTIFY'),$this->data['transaction']);
	}
}