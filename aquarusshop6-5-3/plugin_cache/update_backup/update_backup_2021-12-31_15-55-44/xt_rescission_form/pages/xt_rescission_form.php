...
if (ACTIVATE_XT_RESCISSION_FORM == 'true' && isset($xtPlugin->active_modules['xt_rescission_form'])) {
	...
	if (is_array($_POST) && isset($_POST['action'])) {
		...
		if ($send_mail) {
			...
			$mail->_addBCC($admin_email_address,_STORE_NAME);
	...
	$tpl = 'rescission_form.html';
    $template = new Template();
	$template->getTemplatePath($tpl, 'xt_rescission_form', '', 'plugin');
    ($plugin_code = $xtPlugin->PluginCode('plugin_xt_rescission_form.php:tpl_data')) ? eval($plugin_code) : false;
    $page_data = $template->getTemplate('xt_rescission_form_smarty', $tpl, $tpl_data);
} else {
    $show_page = false;
}
