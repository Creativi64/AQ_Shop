 ...
 * Usps
 *
 * @example   use in templates like {hook key=ew_evelations_usp}
 *
 */
...
if (class_exists('ew_evelations\plugin') && ew_evelations_plugin::status()) {
    ...
    if (ew_evelations_plugin::show_usp()) {
        $tpl_object = new template();
        $tpl = 'ew_evelations_usp.html';
        $tpl_data = array();
        ...
        if (!ew_evelations_plugin::isFileCacheAllowed() || !$tpl_object->isTemplateCache($tpl)) {
            ...
            $dataFormatted = null;
            if ($data = $ew_evelations_plugin->getContentsByBlock('ew_evelations_usp', false)) {
            ...
            $output = $tpl_object->getTemplate('ew_evelations_usp', $tpl, $tpl_data);
        } else {
