...
class easy_credit
{
    ...
    function easy_credit_initVorgang($amount, $logistikDienstleister = '')
    {
        ...
        try
        {
            $kundeSeit = DateTime::createFromFormat('Y-m-d H:i:s', $custInitData->kundeSeit);
            if($kundeSeit) $kundeSeit = $kundeSeit->format('Y-m-d');
        ...
        catch(Exception $e)
        {
            ...
            {
                error_log('xt_easy_credit - init - could not set kundeseit with given value ['.$custInitData->kundeSeit.']. using today\'s date');
                $kundeSeit = new DateTime();
                $kundeSeit = $kundeSeit->format('Y-m-d');
            }
        }
        ...
        else
        {
            ...
            if(is_array($resArray) && is_array($resArray['wsMessages']) && is_array($resArray['wsMessages']['messages']) && count($resArray['wsMessages']['messages']))
            {
    ...
    function getRestbetrag()
    {
        ...
        else
        {
            ...
            $log_data['class'] = 'error_getDecision';
            $log_data['orders_id'] = 0;
            $log_data['error_msg'] = is_array($resArray) && is_array($resArray['wsMessages']) && count($resArray['wsMessages']['messages']) ? $resArray['wsMessages']['messages'][0]['renderedMessage'] : 'Unknown error';
            $log_data['error_data'] = array('vorgangskennung' => $_SESSION['easy_credit']['vorgangskennung'], 'result' => $result);;
            $this->_addCallbackLog($log_data);
