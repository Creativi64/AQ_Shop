    ...
    function _set ($data, $set_type = 'edit')
    {
                ...
                switch ($data['refunds_type']) {
                    case 'Full':
                        if ($o_total != $data['total'] || $data['total'] == 0 || $o_total < $data['total']) {
                            //$obj->failed = false;
                            $obj->error_message = __define('PP_INVALID_REFUND_AMOUNT').' '.$o_total.'#'.$data['total'];
                            return $obj;
                        }
                        break;
        ...
        if ($objC->success) {
            ...
            if ($send_refund == true) {
                ...
                $data_r['total'] = number_format(round($this->getRefundTotal($data['refunds_id']), $currency->decimals), $currency->decimals, '.', ',');
                ...
                if($res['ACK'] == 'Success')
                {
                    ...
                    $obj->success = true;
                    $obj->failed = false;
                    $obj->message = __define('TEXT_PAYPAL_REFUNDS').': '.__define('TEXT_SUCCESS');
                }
                else {
                    $obj->failed = true;
                    $obj->success = false;
