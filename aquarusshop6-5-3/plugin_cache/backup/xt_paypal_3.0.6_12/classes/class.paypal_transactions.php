...
class paypal_transactions
{
    ...
    function _get ($ID = 0)
    {
        ...
        // begin neu
        $master_query = ' ('.$this->_table.'.module = "xt_paypal" OR '.$this->_table.'.module = "xt_paypal_installments")   AND '.$this->_table.'.orders_id !=0 AND '.$this->_table.'.transaction_id != "" AND '.$this->_table.'.class NOT LIKE "callback_data_paypal_refund"';

        $ad_table = '';
        $where_ar = [];
        require_once _SRV_WEBROOT . _SRV_WEB_PLUGINS. "xt_paypal/classes/class.paypalpost.php";
        if(is_array($where_ar) && count($where_ar)>0)
        {
            $master_query .= ' AND ' . implode(" AND ", $where_ar);
        }

        if ($this->url_data['get_data'] || $ID) {
            $table_data = new adminDB_DataRead($this->_table . $ad_table, $this->_table_lang, $this->_table_seo, $this->_master_key, $master_query);
        } else
            $table_data = new adminDB_DataRead($this->_table , $this->_table_lang, $this->_table_seo, $this->_master_key, $master_query);
        // ende neu
