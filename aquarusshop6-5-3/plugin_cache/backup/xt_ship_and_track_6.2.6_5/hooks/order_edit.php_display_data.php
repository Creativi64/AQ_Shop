...
$shipmentPanel->setId('shipmentPanel'.$this->oID)
    ->setActiveTab(0)->setAutoHeight(true)->setBodyStyle('min-height:180px;overflow:scroll')
...
if(!empty(XT_HERMES_USER) && !empty(XT_HERMES_PWD))
{
    $shipmentPanel->addItem(xt_ship_and_track::getAddParcelPanel($this->oID));
}
if(!empty(XT_SHIPCLOUD_API_KEY))
{
    //$shipmentPanel->addItem(xt_ship_and_track::getAddParcelPanel_shipcloud($this->oID));
}
...
$layout->setLayout(new PhpExt_Layout_BorderLayout())
    ...
    ->setAutoWidth(false)->setAutoHeight(true)->setBodyStyle('min-height:250px;')
