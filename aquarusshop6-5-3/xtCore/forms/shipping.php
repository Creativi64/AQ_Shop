...
if ($_POST['action']=='query')
{
    ...
    {
        $sel_country = $_SESSION['geoip_country'];
    }
}
        ...
        if (is_array($value['costs']) && count($value['costs']))
            {
                ...
                {
                    if($value['use_shipping_zone'] == 1 && ($cvalue['shipping_geo_zone'] == '0' || empty($cvalue['shipping_geo_zone']))) {
                        unset($value['costs'][$ckey]);
                        continue;
                    }
            ...
            if ($value['hasCostForCountry'] && false)
            {
